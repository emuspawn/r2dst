...
public class TileV1
{
	...
	public TileV1(int x, int y, int length)
	{
	...
	public int getFScore(Location destination, Unit u)
	{
		if((type == 1 || type == 2) && type != u.getMovementType())
		{
			System.out.println("f score = -1");
			return -1;
		}
		double term1 = destination.x - bounds.x;
		double term2 = destination.y - bounds.y;
		double distance = Math.sqrt((term1*term1)+(term2*term2));
		System.out.println("f score = "+distance);
		return (int)distance;
	}
	public int getFScoreV2(Point dti, Unit u, boolean print)
	{
		if((type == 1 || type == 2) && type != u.getMovementType())
		{
			if(print)
			{
				System.out.println("f score = 99999");
			}
			return 99999;
		}
	...
	public int getFScoreV3(Point dti, Unit u, boolean[][] checkedTiles)
	{
		if((type == 1 || type == 2) && type != u.getMovementType())
		{
